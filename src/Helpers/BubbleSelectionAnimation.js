import clearColors from './HandleColors';

export default function bubbleSelection(steps, setBars, interval, setIsSorting, setIsSorted) {
  let prevBars = false;
  setIsSorting(true);
  steps.forEach((step, i) => {
    setTimeout(() => {
      setBars((bars) => {
        const copyBars = [...bars];
        if (step[2]) {
          [copyBars[step[0]], copyBars[step[1]]] = [copyBars[step[1]], copyBars[step[0]]];
        }
        if (prevBars) {
          copyBars[prevBars[0]].background = 'red';
          copyBars[prevBars[1]].background = 'red';
        }
        copyBars[step[0]].background = 'blue';
        copyBars[step[1]].background = 'blue';
        prevBars = [step[0], step[1]];
        return copyBars;
      });
      if (i === steps.length - 1) {
        setIsSorting(false);
        clearColors(prevBars, setBars);
        setIsSorted(true);
      }
    }, i * interval);
  });
}
